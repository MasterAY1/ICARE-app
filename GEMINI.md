...
9. **Period Collection, Full Payoff & Excess Payment Invariant**: `Actual Collection (Period)` represents total cash repayments received in the period. When a client pays off their loan in full (e.g. Active Credit = ₦150k, Loan Balance = ₦50k, Expected Installment = ₦2.5k, and pays ₦50k): Full Payment is recorded as **₦150,000** (Active Credit settled, count +1) and Excess Payment is ALSO recorded as **₦47,500** (surplus cash $\text{paid} - \text{expected}$, count +1). Base scheduled collections are derived as $\text{Actual Collection} - \text{Excess Payments}$. Every payoff and excess event must be permanently recorded in the dedicated `loan_payoff_excess_records` database table. See [DASHBOARD_AND_REPORTING_RULES.md](file:///c:/Users/DELL/Desktop/Master_%20AY%20Projects/trustmicro-credit/.agents/rules/business-rules/DASHBOARD_AND_REPORTING_RULES.md) (`BR-DASH-005`, `BR-DASH-007`).
10. **Strict Zero-Emoji Governance (SVG Icons Only)**: Emojis are strictly prohibited anywhere in the system UI (tab headers, buttons, cards, expanders, tables, alerts, dialogs, form controls, or markdown text). Do NOT use emoji characters. Use clean, professional corporate typography or SVG icons instead.
11. **Dual-Frontend Parity & Zero-Merge Governance**: When modifying business logic, calculations, or workflows, changes MUST apply simultaneously to both Streamlit (production on GitHub) and Flutter (local/staging pilot) while strictly prohibiting any git branch merges between `main` and migration branches without explicit user approval. See [dual-frontend-parity](file:///c:/Users/DELL/Desktop/Master_%20AY%20Projects/trustmicro-credit/.agents/skills/dual-frontend-parity/SKILL.md).
