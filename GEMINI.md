...
9. **Period Collection Mathematical Balance Invariant**: $\text{Actual Collection (Period)} = \text{Normal Payments} + \text{Excess Payments} + \text{Part Payments}$. When all scheduled clients pay their expected installments: $\text{Expected Repayment (Period)} = \text{Actual Collection (Period)} = \text{Normal Payments}$, with $\text{Excess Payments} = ₦0.00$, $\text{Part Payments} = ₦0.00$, and $\text{Not Paid} = ₦0.00$. See [DASHBOARD_AND_REPORTING_RULES.md](file:///c:/Users/DELL/Desktop/Master_%20AY%20Projects/trustmicro-credit/.agents/rules/business-rules/DASHBOARD_AND_REPORTING_RULES.md) (`BR-DASH-007`).
