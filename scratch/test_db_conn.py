...
regions = ["us-east-1", "eu-west-1", "eu-central-1", "ap-southeast-1", "us-west-1", "sa-east-1"]
hosts = [f"aws-0-{r}.pooler.supabase.com" for r in regions] + ["db.zkphpwixqpebsctfklpd.supabase.co", "54.210.158.42"]
password = "Icare@2026"
dbname = "postgres"
...
for host in hosts:
    for port in [5432, 6543]:
        for user in ["postgres.zkphpwixqpebsctfklpd", "postgres"]:
            try:
                print(f"Connecting to host={host}, port={port}, user={user}...")
