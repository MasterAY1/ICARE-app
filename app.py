...
if page == "Dashboard":
    ...
    with SupabaseUnitOfWork() as uow_p:
        ...
        r3.metric("Excess Payments", f"{p_sum.get('excess_payments', {}).get('count', 0)} (₦{p_sum.get('excess_payments', {}).get('amount', 0.0):,.0f})")
        r4.metric("Overdue Clients", f"{p_sum.get('overdue', {}).get('count', 0)} (₦{p_sum.get('overdue', {}).get('amount', 0.0):,.0f})")
        r5.metric("Portfolio PAR", p_sum.get('par', '0.00%'))
