...
if page == "Dashboard":
    ...
    with SupabaseUnitOfWork() as uow_p:
        ...
        c1.metric("Registered", f"{reg_count} Clients", help=f"Total Roster: {tot_count} Clients")
        ...
        s1, s2, s3 = st.columns(3)
        s1.metric("Savings Deposited (Period)", f"₦{p_sum.get('period_savings_deposit', 0.0):,.0f}", help=f"{p_sum.get('period_savings_dep_clients', 0)} Clients")
        s2.metric("Savings Withdrawn (Period)", f"₦{p_sum.get('period_savings_withdrawal', 0.0):,.0f}", help=f"{p_sum.get('period_savings_wd_clients', 0)} Clients")
        s3.metric("Total Savings Balance", f"₦{p_sum.get('total_savings_balance', 0.0):,.0f}", help=f"{p_sum.get('total_savings_clients', 0)} Active Savers")
        ...
        d1.metric("Loans Disbursed", f"{d_sum['count']} Loans", help=f"{d_sum.get('client_count', d_sum['count'])} Clients")
        d2.metric("Total Amount Disbursed", f"₦{d_sum['amount']:,.0f}", help=f"{d_sum['count']} Loans Disbursed")

        st.caption("Row 4: Loan & Collection Summary")
        l1, l2, l3, l4 = st.columns(4)
        l1.metric("Total Active Credit", f"₦{p_sum.get('total_active_credit', 0.0):,.0f}", help=f"{p_sum.get('active_loans_count', 0)} Active Loans")
        l2.metric("Expected Repayment", f"₦{p_sum.get('total_expected_repayment', 0.0):,.0f}", help=f"{p_sum.get('expected_repay_clients', 0)} Clients")
        l3.metric("Actual Collections (Period)", f"₦{p_sum.get('total_actual_collection', p_sum.get('today_collection', 0.0)):,.0f}", help=f"{p_sum.get('paying_clients_count', 0)} Clients")
        l4.metric("Total Outstanding Balance", f"₦{p_sum.get('total_outstanding_balance', 0.0):,.0f}", help=f"{p_sum.get('outstanding_clients_count', 0)} Clients")

        st.caption("Row 5: Repayment Status & Risk")
        r1, r2, r3, r4, r5 = st.columns(5)
        r1.metric("Normal Payments", f"₦{p_sum.get('normal_payments', {}).get('amount', 0.0):,.0f}", help=f"{p_sum.get('normal_payments', {}).get('count', 0)} Clients")
        r2.metric("Full Payments", f"₦{p_sum.get('full_payments', {}).get('amount', 0.0):,.0f}", help=f"{p_sum.get('full_payments', {}).get('count', 0)} Clients")
        r3.metric("Excess Payments", f"₦{p_sum.get('excess_payments', {}).get('amount', 0.0):,.0f}", help=f"{p_sum.get('excess_payments', {}).get('count', 0)} Clients")
        r4.metric("Overdue Portfolio", f"₦{p_sum.get('overdue', {}).get('amount', 0.0):,.0f}", help=f"{p_sum.get('overdue', {}).get('count', 0)} Overdue Clients")
        par_val = float(str(p_sum.get('par', '0.00%')).replace('%', '') or 0.0)
        r5.metric("Portfolio at Risk (PAR)", f"{par_val:.2f}%", delta=f"{par_val:.1f}% Risk" if par_val > 0 else None, delta_color="inverse" if par_val > 5.0 else "normal")
